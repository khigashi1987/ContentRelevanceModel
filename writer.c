...
void crm_write(FILE *pp, FILE *tp, FILE *sp, FILE *n_mzp, FILE *n_wzp, FILE *m_mzp, FILE *m_tzp, double **phi, double **theta, double **psi, int **n_mz, int **n_zw, int **m_mz, int **m_zt, int nclass, int nlex, int ntlex, int ndoc){
    printf("writing model...\n"); fflush(stdout);
    write_matrix(pp, phi, nlex, nclass);
    write_matrix(tp, theta, ndoc, nclass);
    write_matrix(sp, psi, ntlex, (nclass+1));
    write_imatrix(n_mzp, n_mz, ndoc, nclass);
    write_imatrix_transpose(n_wzp, n_zw, nclass, nlex);
    write_imatrix(m_mzp, m_mz, ndoc, (nclass+1));
    write_imatrix_transpose(m_tzp, m_zt, (nclass+1), ntlex);
    printf("done.\n"); fflush(stdout);
}
