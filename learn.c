...
void crm_learn(document *data, document *tag, double alpha, double beta, double gamma, double eta, int nclass, int nlex, int ntlex, int dlenmax, int tlenmax, int maxiter, double **phi, double **theta, double **psi, int **n_mz, int **n_zw, int **m_mz, int **m_zt, FILE *likp, FILE *hyperp){
    ...
    for(dp = tag, ntdocs = 0;(dp->len) != -1;dp++, ntdocs++)
        ;
    if(ndocs != ntdocs){
        fprintf(stderr,"crm_learn:: size of documents and tagsets are different.\n");
        return;
    }
    
    // initialize buffers
    if((n_m = calloc(ndocs,sizeof(int))) == NULL){
        fprintf(stderr,"crm_learn:: cannot allocate n_m.\n");
        return;
    }
    if((n_z = calloc(nclass,sizeof(int))) == NULL){
        fprintf(stderr,"crm_learn:: cannot allocate n_z.\n");
        return;
    }
    if((m_m = calloc(ndocs,sizeof(int))) == NULL){
        fprintf(stderr,"crm_learn:: cannot allocate m_m.\n");
        return;
    }
    if((m_z = calloc((nclass+1),sizeof(int))) == NULL){
        fprintf(stderr,"crm_learn:: cannot allocate m_z.\n");
        return;
    }
    if((left = calloc(nclass,sizeof(double))) == NULL){
        fprintf(stderr,"crm_learn:: cannot allocate left.\n");
        return;
    }
    if((center = calloc(nclass,sizeof(double))) == NULL){
        fprintf(stderr,"crm_learn:: cannot allocate center.\n");
        return;
    }
    if((right = calloc(nclass,sizeof(double))) == NULL){
        fprintf(stderr,"crm_learn:: cannot allocate right.\n");
        return;
    }
    if((p_z = calloc(nclass,sizeof(double))) == NULL){
        fprintf(stderr,"crm_learn:: cannot allocate p_z.\n");
        return;
    }
    if((cum_sum_p_z = calloc((nclass+1),sizeof(double))) == NULL){
        fprintf(stderr,"crm_learn:: cannot allocate cum_sum_p_z.\n");
        return;
    }
    if((topics = calloc(ndocs,sizeof(int **))) == NULL){
        fprintf(stderr,"crm_learn:: cannot allocate topics.\n");
        return;
    }
    if((tagtopics = calloc(ndocs,sizeof(int **))) == NULL){
        fprintf(stderr,"crm_learn:: cannot allocate tagtopics.\n");
        return;
    }
    if((temp_phi = dmatrix(nlex, nclass)) == NULL){
        fprintf(stderr,"crm_learn:: cannot allocate temp_phi.\n");
        exit(1);
    }
    if((temp_theta = dmatrix(ndocs, nclass)) == NULL){
        fprintf(stderr,"crm_learn:: cannot allocate temp_theta.\n");
        exit(1);
    }
    if((temp_psi = dmatrix(ntlex, (nclass+1))) == NULL){
        fprintf(stderr,"crm_learn:: cannot allocate temp_psi.\n");
        exit(1);
    }
    ...
    for(dp = data, m = 0;(dp->len) != -1;dp++, m++){
        ...
        for(w = 0;w < (dp->len);w++){
            if((topics[m][w] = calloc((dp->cnt[w]), sizeof(int))) == NULL){
                fprintf(stderr,"crm_learn:: cannot allocate topics[m][w].\n");
                return;
            }
            word_index = dp->id[w];
            word_num = dp->cnt[w];
    ...
    for(tp = tag, m = 0;(tp->len) != -1;tp++, m++){
        if((tagtopics[m] = calloc((tp->len), sizeof(int *))) == NULL){
            fprintf(stderr, "crm_learn:: cannot allocate tagtopics[m].\n");
            return;
        }
        for(t = 0;t < (tp->len);t++){
            if((tagtopics[m][t] = calloc((tp->cnt[t]), sizeof(int))) == NULL){
                fprintf(stderr, "crm_learn:: cannot allocate tagtopics[m][t].\n");
                return;
            }
            tag_index = tp->id[t];
            tag_num = tp->cnt[t];
            for(i = 0;i < tag_num;i++){
                Mall += 1;
                if(gsl_rng_uniform(r) < eta){ // content-relevant tag
                    sum_p_z = 0.0;
                    for(k = 0;k < nclass;k++){
    ...
    for(it = 0;it < maxiter;it++){
        ...
        for (dp = data, tp = tag, m = 0; (dp->len) != -1; dp++, tp++, m++){
            ...
            for(t = 0;t < (tp->len);t++){
                ...
                for(i = 0;i < tag_num;i++){
                    ...
                    Mall -= 1;
                    M0 = m_z[nclass];
                    M_0t = m_zt[nclass][tag_index];
                    p_r0 = (((double)M0 + eta) / ((double)Mall + 2.0*eta)) * (((double)M_0t + gamma) / ((double)M0 + gamma * (double)ntlex));
                    p_r1 = (((double)Mall - (double)M0 + eta) / ((double)Mall + 2.0*eta)) * (((double)m_zt[z][tag_index] + gamma) / ((double)m_z[z] + gamma * (double)ntlex));
                    sum_p_r = p_r0 + p_r1;
                    p_r1 /= sum_p_r;
                    if(gsl_rng_uniform(r) < p_r1){ // content relevant
                        // p_z left
                        for(k = 0;k < nclass;k++)
        ...
        alpha = update_alpha(alpha, n_m, n_mz, ndocs, nclass);
        beta = update_beta(beta, n_z, n_zw, nclass, nlex);
        gamma = update_gamma(gamma, m_z, m_zt, (nclass+1), ntlex);
        eta = update_eta(eta, M0, (Mall-M0));
        
        // compute likelihood.
        lik = loglikelihood(n_mz, n_zw, n_m, m_mz, m_zt, m_z, nclass, nlex, ntlex, ndocs, Mall, alpha, beta, gamma, eta);
        printf("\tlikelihood ... %.8f\n",lik);
        printf("\talpha ... %.2f, beta ... %.2f, gamma ... %.2f, eta ... %.2f\n",alpha,beta,gamma,eta);
        fprintf(likp,"%.8f\n",lik);
        fprintf(hyperp,"%.8f,%.8f,%.8f,%.8f\n",alpha,beta,gamma,eta);
    }
    ...
    normalize_matrix_col(psi, temp_psi, ntlex, (nclass+1));
